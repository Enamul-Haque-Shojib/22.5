...
class TransferMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if account_number == user_account.account_no:
            messages.warning(
            self.request,
            ...
            check_account = UserBankAccount.objects.filter(account_no=account_number)
        
        if check_account:
            transfer_account = UserBankAccount.objects.get(account_no=account_number)
            ...
            send_transaction_email(transfer_account.user, amount, "Transfer Money Message", "transactions/transfer_account_email.html")
        else:
            print(">>>>>>>>>>>>>>> Not found")
            messages.warning(
            self.request,
            f'The account is not found'
            )
        ...
        messages.success(
            self.request,
            f'Successfully transfer {"{:,.2f}".format(float(amount))}$ from your account'
        )
